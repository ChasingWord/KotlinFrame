...
object ActivityUtil {
    ...
    private fun sendNotificationBelow26(
        ...
        builder.setAutoCancel(true) //点击后让通知将消失
            ...
            .setSmallIcon(R.drawable.smlogo)
            ...
            return if (permission.isNotEmpty()) {
                ...
                for (permissionItem in permission) {
                    if (ActivityCompat.checkSelfPermission(
                        ...
                        if (!ActivityCompat.shouldShowRequestPermissionRationale(
                            ...
                            var permissionLocal: Any?
                            when (permissionItem) {
                                Manifest.permission.WRITE_EXTERNAL_STORAGE -> {
                                    permissionLocal = ObjectCacheUtil.readObject(
                                        activity,
                                        "WRITE_EXTERNAL_STORAGE",
                                        false
                                    )
                                    if (permissionLocal as Boolean && !permissionLocal) {
                                        needRequestPermission.add(permissionItem)
                                        ObjectCacheUtil.saveObject(
                                            activity,
                                            "WRITE_EXTERNAL_STORAGE",
                                            true
                                        )
                                    } else showToast(activity, "请前往手机系统设置界面进行设置存储权限！")
                                }
                                ...
                                Manifest.permission.CAMERA -> {
                                    permissionLocal =
                                        ObjectCacheUtil.readObject(activity, "CAMERA", false)
                                    if (permissionLocal as Boolean && !permissionLocal) {
                                        needRequestPermission.add(permissionItem)
                                        ObjectCacheUtil.saveObject(activity, "CAMERA", true)
                                    } else showToast(activity, "请前往手机系统设置界面进行设置拍照权限！")
                                }
                                Manifest.permission.CALL_PHONE -> {
                                    permissionLocal =
                                        ObjectCacheUtil.readObject(activity, "CALL_PHONE", false)
                                    if (permissionLocal as Boolean && !permissionLocal) {
                                        needRequestPermission.add(permissionItem)
                                        ObjectCacheUtil.saveObject(activity, "CALL_PHONE", true)
                                    } else showToast(activity, "请前往手机系统设置界面进行设置拨打电话权限！")
                                }
                                Manifest.permission.WRITE_CALENDAR -> {
                                    permissionLocal =
                                        ObjectCacheUtil.readObject(
                                            activity,
                                            "WRITE_CALENDAR",
                                            false
                                        )
                                    if (permissionLocal as Boolean && !permissionLocal) {
                                        needRequestPermission.add(permissionItem)
                                        ObjectCacheUtil.saveObject(activity, "WRITE_CALENDAR", true)
                                    } else showToast(activity, "请前往手机系统设置界面进行设置读写日历权限！")
                                }
                                else -> showToast(activity, "请前往手机系统设置界面进行设置相应权限！")
                            }
                ...
                if (CollectionUtil.isNotEmpty(needRequestPermission)) {
                    CoroutineScope(Dispatchers.Main).launch {
                        try {
                            activity.requestPermissions(needRequestPermission.toTypedArray(), 0)
                        } catch (e: Exception) {
